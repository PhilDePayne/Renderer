	...
	std::vector<std::pair<unsigned int, float>>& pixelColorsWeight, int rep, Scene& scene)
{
	...
	for (int k = 0; k < scene.elements.size(); k++) {
		...
		IntersectionResult ir1 = scene.elements[k]->hit(r1);
		...
		IntersectionResult ir2 = scene.elements[k]->hit(r2);
		...
		IntersectionResult ir4 = scene.elements[k]->hit(r4);
		...
		IntersectionResult ir5 = scene.elements[k]->hit(r5);
