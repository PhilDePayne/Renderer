...
void Cylinder::generate() {
	...
	vertices.push_back(center + vec3f(0.0f, height / 2, 0.0f));
	...
	for (int y = 0; y < segmentsY + 1; y++) {
		for (float x = 0; x < 2 * PI; x += 2 * PI / segmentsX) {
			...
			vertices.push_back(vec3f(radius * cos(x) + center.x, y * (height/segmentsY) - (center.y - height/2), radius * sin(x) + center.z));

		}
		vertices.push_back(vec3f(radius * cos(0) + center.x, y * (height / segmentsY) - (center.y - height / 2), radius * sin(0) + center.z));
	}
	...
	{
		printf("\n %f %f %f", vertex.x, vertex.y, vertex.z);
	}
